...
coa_courses <- coa_courses %>%
  ...
  mutate(Degree_Requirements = str_replace(Degree_Requirements, pattern = "\\.", replacement = "")) %>%
  mutate(Degree_Requirements = str_replace_all(Degree_Requirements, pattern = " |\\*", replacement = "")) %>%
...
coa_courses3 %>%
  separate_rows(Degree_Requirements, sep = ", | |,") %>%
  distinct(Degree_Requirements, CourseNumber, Year_Term, .keep_all = TRUE) %>% # This will be added to drop duplicates
...
coa_courses3 %>%
  separate_rows(CourseDivision, sep = ", | |,") %>%
  distinct(Degree_Requirements, CourseNumber, Year_Term, .keep_all = TRUE) %>% # This will be added to drop duplicates
...
```{r graduation requirement proportion per year}
...
coa_courses3 %>%
  separate_rows(CourseDivision, sep = ", | |,") %>%
  distinct(Degree_Requirements, CourseNumber, Year_Term, .keep_all = TRUE) %>% # This will be added to drop duplicates
  mutate(Requirement_opportunity = CourseDivision) %>%
...
coa_courses3 %>%
  separate_rows(CourseDivision, sep = ", | |,") %>%
  distinct(Department, CourseDivision, CourseNumber, Year_Term, .keep_all = TRUE) %>% # This will be added to drop duplicates
  mutate(Requirement_opportunity = CourseDivision) %>%
  # mutate(Requirement_opportunity = case_when(
  #   CourseDivision %in% c("AD", "AD HY", "AD HY WFO", "AD WFO") ~ "AD",
  #   CourseDivision %in% c("ADS", "ADS HY") ~ "ADS",
  ...
  # )) %>%
  filter(!is.na(Requirement_opportunity)) %>%
  filter(Requirement_opportunity %in% c("W", "ES", "ED", "HS", "HY", "QR", "AD", "ADS")) %>%
...
ggsave(filename = "Number_Courses_Available.png", width = 10, height = 7)
```
  ...
  mutate(instructor_status = case_when(Instructor %in% c("Anderson, John", 
                          ...
                          Instructor %in% c("Lakey, Heather") & Year > 2021 ~ "Permanent Faculty",
                            ...
                            Instructor %in% c("Cass, Blake", 
                                              ...
                                              "Gibson, David",
                                              "Teaching Staff",
                                              "Longsworth, Gordon", "Graham, Carrie") ~ "Teaching Staff",
...
```{r Laurie - exploring courses taught by permanent faculty and adjuncts that could meet the degree requirement}

coa_courses3 %>%
  group_by(Instructor) %>% 
  mutate(Num_Courses_Taught = 1:n()) %>%
  ungroup() %>%
  filter(Num_Courses_Taught > 3, instructor_status %in% c("Adjunct"),
           is.na(Degree_Requirements) == TRUE | Degree_Requirements %in% c("None")) %>%
  group_by(Year, Department, instructor_status) %>%
  summarize(count = n()) %>%
  ggplot() +
  geom_line(aes(x = Year, y = count, color = Department)) +
  labs(title = "Number adjunct and lecturer courses eligible under 3 course rule") +
  facet_wrap(~instructor_status, nrow = 1)


ggsave(filename = "Number_Courses_Eligible_Under_Three_Course_Rule.png", width = 10, height = 7)

coa_courses3 %>%
  group_by(Instructor) %>% 
  mutate(Num_Courses_Taught = 1:n()) %>%
  ungroup() %>%  arrange(fct_relevel(instructor_status, "Permanent Faculty")) %>%
  separate_rows(Degree_Requirements, sep = ", | |,") %>%
  mutate(Academic_Year = case_when(str_detect(Year_Term, pattern = "FA") ~ paste0("AY ", Year, "-", Year+1),
                                   TRUE ~ paste0("AY ",Year-1, "-", Year))) %>%
  distinct(Degree_Requirements, CourseNumber, Year_Term, Academic_Year, .keep_all = TRUE) %>%
  filter(Num_Courses_Taught > 3 & instructor_status == "Adjunct") %>%
  group_by(Department, Degree_Requirements, Year_Term) %>%
  count()
  

coa_courses3 %>%
  separate_rows(Degree_Requirements, sep = ", | |,") %>%
  arrange(fct_relevel(instructor_status, "Permanent Faculty")) %>%
  mutate(Academic_Year = case_when(str_detect(Year_Term, pattern = "FA") ~ paste0("AY ", Year, "-", Year+1),
                                   TRUE ~ paste0("AY ",Year-1, "-", Year))) %>%
  distinct(Degree_Requirements, CourseNumber, Year_Term, Academic_Year, .keep_all = TRUE) %>% # This will be added to drop duplicates
  filter(Degree_Requirements %in% c("ES", "HS", "AD", "ADS")) %>%
  group_by(Academic_Year, Degree_Requirements, instructor_status) %>%
  summarize(count = n()) %>%
  ggplot(aes(x = fct_rev(Academic_Year), 
             y = count, 
             fill = fct_rev(fct_infreq(instructor_status)))) +
  geom_col() +
  labs(title = "Number of Courses Available That Meet Requirements Over the Year", 
       subtitle = "By Instructor Status",
       x = "Academic Year", 
       y = "Number of Courses",
       fill = "Instructor Status") +
  scale_colour_viridis_d() +
  facet_wrap(~Degree_Requirements)  +
  coord_flip()

ggsave(filename = "Number_Courses_Meeting_HS_AD_ADS_ES.png", width = 10, height = 7)
...
coa_courses3 %>%
  ...
  distinct(Degree_Requirements, CourseNumber, Year_Term, Academic_Year, .keep_all = TRUE) %>% # This will be added to drop duplicates
  filter(Degree_Requirements %in% c("ES", "HS", "AD", "ADS")) %>%
  mutate(Degree_Requirements = case_when(Degree_Requirements %in% c("AD", "ADS") ~ "AD or ADS",
                                         TRUE ~ Degree_Requirements)) %>%
  group_by(Academic_Year, Degree_Requirements, instructor_status) %>%
  summarize(seats = sum(as.numeric(Class_Size), na.rm = TRUE)) %>%
  ggplot(aes(x = fct_rev(Academic_Year), 
             y = seats, 
             fill = fct_rev(fct_infreq(instructor_status)))) +
  geom_col() +
  labs(title = "Number of Seats Available That Meet Requirements Over the Year", 
       subtitle = "By Instructor Status",
       x = "Academic Year", 
       y = "Number of Seats",
       fill = "Instructor Status") +
...
ggsave(filename = "Number_Seats_Meeting_HS_AD_ADS_ES.png", width = 10, height = 7)
...
coa_courses3 %>%
  ...
  mutate(Academic_Year = case_when(str_detect(Year_Term, pattern = "FA") ~ paste0("AY ", Year, "-", Year+1),
                                   TRUE ~ paste0("AY ",Year-1, "-", Year))) %>%
  distinct(Degree_Requirements, CourseNumber, Year_Term, Academic_Year, .keep_all = TRUE) %>% # This will be added to drop duplicates
  filter(Degree_Requirements %in% c("QR", "HY", "W")) %>%
  group_by(Academic_Year, Degree_Requirements, instructor_status) %>%
  summarize(seats = sum(as.numeric(Class_Size), na.rm = TRUE)) %>%
  ggplot(aes(x = fct_rev(Academic_Year), 
             y = seats, 
...
ggsave(filename = "Number_Seats_Meeting_QR_W_HY.png", width = 10, height = 7)
...
coa_courses3 %>%
  ...
  mutate(Degree_Requirements = case_when(Degree_Requirements %in% c("AD", "ADS") ~ "AD or ADS",
                                         TRUE ~ Degree_Requirements)) %>%
  ggplot(aes(y = Degree_Requirements, 
             x = as.numeric(Class_Size),
             fill = Degree_Requirements)) +
  geom_density_ridges() +
  labs(title = "Number of Courses Available That Meet Requirements Over the Year", 
       subtitle = "By Instructor Status",
       y = "Academic Year", 
...
ggsave(filename = "Class_Limit_Meeting_HS_AD_ADS_ES.png", width = 10, height = 7)
...
coa_courses3 %>%
  ...
  mutate(Academic_Year = case_when(str_detect(Year_Term, pattern = "FA") ~ paste0("AY ", Year, "-", Year + 1),
                                   TRUE ~ paste0("AY ",Year-1, "-", Year))) %>%
  distinct(Degree_Requirements, CourseNumber, Year_Term, Academic_Year, .keep_all = TRUE) %>% # This will be added to drop duplicates
  filter(Degree_Requirements %in% c("QR"), Year == 2022) %>%
  group_by(Year_Term, Degree_Requirements, Instructor, instructor_status) %>%
  summarize(count = n()) 
...
coa_courses3 %>%
  ...
  mutate(Academic_Year = case_when(str_detect(Year_Term, pattern = "FA") ~ paste0("AY ", Year, "-", Year+1),
                                   TRUE ~ paste0("AY ",Year-1, "-", Year))) %>%
  distinct(Degree_Requirements, CourseNumber, Year_Term, Academic_Year, .keep_all = TRUE) %>% # This will be added to drop duplicates
  filter(Degree_Requirements %in% c("W", "QR", "HY")) %>%
  group_by(Academic_Year, Degree_Requirements, instructor_status) %>%
  summarize(count = n()) %>%
  ggplot(aes(x = fct_rev(Academic_Year), 
             y = count, 
...
ggsave(filename = "Number_Courses_Meeting_QR_W_HY.png", width = 10, height = 7)
```
...
```{r courses per area and instructors animations}
coa_courses3 %>%
  filter(!Department %in% c('SG', 'GS', 'HE')) %>%
  ggplot() +
...
```{r course distributions over terms}
# i want a code that shows all the classes in a term by department and then animate over time

# coa_courses3$Department_combined <- ifelse(coa_courses3$Department == "HE", "HS", coa_courses3$Department)
...
```{r initial animation}
P2 <- ggplot(subset(coa_courses3, Department != "SG" & Department != "GS" & Department != "HE"), aes(y = fct_rev(fct_relevel(Year_Term, c("18WI", "18SP",
        "18FA", "19WI", "19SP",
        "19FA", "20WI", "20SP",
        "20FA", "21WI", "21SP",
        "21FA", "22WI", "22SP",
...
P2 +  geom_bar(aes(fill = fct_relevel(Department, c("MD", "ED","AD", "ES", "HS" ))), position = "fill") +
    labs(
    title = "Distribution of Courses by Department for Each Term",
    x = "Course Proportion",
    y = "Term",
  fill = "Department") +
  transition_states(Year_Term, transition_length = 3, state_length = 1,
                    wrap = FALSE)+
  ease_aes('linear') +
  theme_minimal() +
  view_follow()
