...
P1 + facet_wrap(~Department) + 
  geom_bar(aes(fill = instructor_status), position = "fill") + 
  labs(title = "Distribution of Courses Taught in Departments by Different Instructor Status",
  subtitle = "Through the Terms from Winter of 2018 to Spring 2023",
    x = "Course Proportion",
    y = "Term",
  fill = "Instructor Status") 

ggplot(subset(coa_courses3, Department != "SG" & Department != "GS" & Department != "HE"), aes(x = Year_Term, y = after_stat(count), group = Department_combined, color = Department_combined)) +
...
```{r}
P2 <- ggplot(subset(coa_courses3, Department != "SG" & Department != "GS" & Department != "HE"), 
             aes(y = fct_rev(fct_relevel(Year_Term, c("18WI", "18SP", "18FA", "19WI", "19SP", "19FA", 
                                                      "20WI", "20SP", "20FA", "21WI", "21SP", "21FA", 
                                                      "22WI", "22SP", "22FA", "23WI", "23SP"))))) +
  scale_fill_brewer(palette = "RdBu") +
  theme_dark() +
  theme(legend.position = "bottom") 

animated_plot <- P2 +  
  geom_bar(aes(fill = fct_relevel(Department, c("MD", "ED","AD", "ES", "HS" ))), position = "fill") +
  labs(
    title = "Distribution of Courses by Department for Each Term",
    x = "Course Proportion",
    y = "Term",
  ...
  transition_states(Year_Term, transition_length = 3, state_length = 1, wrap = FALSE) +
  ease_aes('linear') +
  theme_minimal() +
  view_follow()
...
ggplot(coa_courses3 %>% filter(!Department %in% c('HE', 'SG', 'GS')), aes(x = Department, y = Class_Size)) +
  geom_bar(stat = "summary", fun = "mean") +
  labs(title = "Class Size by Department", x = "Department", y = "Class Size")

ggplot(coa_courses3 %>% filter(!Department %in% c('HE', 'SG', 'GS')), aes(x = Department, y = Class_Size)) +   geom_violin() + 
